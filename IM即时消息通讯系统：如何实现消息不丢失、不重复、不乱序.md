...
完整的消息状态追踪系统是可靠传输的重要保障。每条消息在系统中都应当有明确的状态标识，如“已创建”“已发送”“已接收”“已送达”“已读”等。

服务器需要维护这些状态并在状态变化时及时更新。当消息在某个环节处理失败时，系统应当能够识别并触发补偿机制。补偿机制可以采用定时扫描的方式，检查长时间未推进状态的消息并重新处理，也可以采用事件驱动的方式，在特定条件触发时自动执行补偿逻辑。
